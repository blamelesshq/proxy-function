...
variable "collectionname" { default = "pgjeorgiev-testing-apim-12345" }
...
resource "azurerm_api_management_api" "example" {
  name                = "blameless-api-29092021-123"
  resource_group_name = azurerm_resource_group.main.name
  api_management_name = azurerm_api_management.example.name
  revision            = "2"
  display_name        = "Blameless API"
  path                = ""
  protocols           = ["https"]
...
#   name                = "blameless-api-backend"
#   resource_group_name = azurerm_resource_group.main.name
#   api_management_name = azurerm_api_management.example.name
#   protocol            = "http"
#   url                 = "https://310920211func.azurewebsites.net/api/"
...
resource "azurerm_api_management_api_policy" "example" {
  ...
  xml_content = <<XML
    <policies>
        <inbound>
            <base />
            <set-backend-service base-url="https://310920211func.azurewebsites.net/api/" />
        </inbound>
    </policies>
  XML
}
